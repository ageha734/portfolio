...
test.describe("Portfolios API", () => {
    test("should return portfolios list", async ({ request }) => {
        const response = await request.post(`${TRPC_ENDPOINT}/portfolios.list`, {
            data: {},
        });
    ...
    test("should return portfolio by slug", async ({ request }) => {
        const listResponse = await request.post(`${TRPC_ENDPOINT}/portfolios.list`, {
            data: {},
        });
        ...
        if (listData.result?.data?.length > 0) {
            ...
            const response = await request.post(`${TRPC_ENDPOINT}/portfolios.bySlug`, {
                data: { slug },
            });
