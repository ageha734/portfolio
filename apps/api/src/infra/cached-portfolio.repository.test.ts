...
describe("CachedPortfolioRepository", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(PortfolioRepositoryImpl).mockImplementation(
			() => mockDbRepository as any,
		);
	});
	...
	describe("constructor", () => {
		...
		test("should create instance with databaseUrl and redisUrl", () => {
			...
			expect(PortfolioRepositoryImpl).toHaveBeenCalledWith(
				"mysql://localhost:3306/db",
			);
			expect(repository).toBeDefined();
		});
	...
	describe("findBySlug", () => {
		test("should return cached value when cache hit", async () => {
			...
			expect(mockCacheService.get).toHaveBeenCalledWith(
				"portfolio:findBySlug:test-portfolio",
			);
		...
		test("should fetch from DB when cache miss", async () => {
			...
			expect(mockDbRepository.findBySlug).toHaveBeenCalledWith(
				"test-portfolio",
			);
			expect(mockCacheService.set).toHaveBeenCalledWith(
				"portfolio:findBySlug:test-portfolio",
