...
export default function CoverageReportsPage() {
    ...
    useEffect(() => {
        async function loadReports() {
            try {
                // static/reports/coverage/ ディレクトリからレポート一覧を取得
                const response = await fetch("/reports/coverage/list.json").catch(() => null);
                if (response?.ok) {
    ...
    return <ReportList type="coverage" reports={reports} />;
}
