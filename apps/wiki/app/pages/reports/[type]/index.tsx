...
export default function ReportsPage() {
    ...
    const type = location.pathname.split("/").pop() as "e2e" | "coverage" | undefined;
    ...
    useEffect(() => {
        ...
        if (type !== "e2e" && type !== "coverage") {
            setError("無効なレポートタイプです。e2e または coverage を指定してください。");
            setLoading(false);
            return;
        }
        ...
        async function loadReports() {
            try {
                const response = await fetch(`/reports/${type}/list.json`).catch(() => null);
                if (response?.ok) {
