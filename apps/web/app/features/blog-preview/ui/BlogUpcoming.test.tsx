...
describe("BlogUpcoming Component", () => {
	...
	test("should render upcoming post items", () => {
		...
		expect(
			screen.getByText("Escape Hatches - brought up in Vercel conf"),
		).toBeInTheDocument();
		expect(
			screen.getByText("We are all salesmen, quick prototypes, sandboxes"),
		).toBeInTheDocument();
		expect(
			screen.getByText("Developer tooling - debuggers & loggers"),
		).toBeInTheDocument();
		expect(screen.getByText("Lerna is back!")).toBeInTheDocument();
		expect(
			screen.getByText("You are not Google - moving quickly"),
		).toBeInTheDocument();
		expect(
			screen.getByText("Reviewing pull requests - a simple framework"),
		).toBeInTheDocument();
	});
