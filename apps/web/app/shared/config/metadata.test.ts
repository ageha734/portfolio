...
describe("metadata", () => {
	describe("getMetaDataBase", () => {
		...
		test("should include charset", () => {
			...
			const charset = result.find(
				(meta): meta is { charset: string } => "charset" in meta,
			);
		...
		test("should include author", () => {
			...
			const author = result.find(
				(meta): meta is { author: string } => "author" in meta,
			);
		...
		test("should include viewport", () => {
			...
			const viewport = result.find(
				(meta): meta is { viewport: string } => "viewport" in meta,
			);
	...
	describe("getMetaData", () => {
		...
		test("should use custom description", () => {
			...
			const descMeta = result.find(
				(meta): meta is { name: string; content: string } =>
		...
		test("should use custom image", () => {
			...
			const imageMeta = result.find(
				(meta): meta is { image: string } => "image" in meta,
			);
