		...
		<section className={cn("relative w-full", className)}>
			<div className="flex flex-col items-center justify-center gap-8 p-12 md:p-20">
				<div className="flex flex-col gap-4">
					<h1 className="font-extrabold font-serif text-4xl md:text-6xl">
						{title}
					</h1>
			...
			>
				{!!backgroundElement && (
					<div
						...
						style={
							{ "--opacity": backgroundOverlayOpacity } as React.CSSProperties
						}
					>
						{backgroundElement}
					</div>
...
export const ProjectBackground = ({
	...
	return (
		<div className={cn("absolute inset-0 overflow-hidden", className)}>
			<div
				className="absolute inset-0 translate-y-(--offset,0px)"
				ref={imageRef}
			>
				{src && (
					<img
...
export const ProjectSectionContent = ({
	...
	<div
		className={cn("mx-auto w-full px-4", widthClasses[width], className)}
		{...rest}
	/>
);
...
export const ProjectSectionHeading = ({
	...
	<As
		className={cn(
			"mb-4 font-extrabold font-serif text-3xl md:text-4xl",
			className,
		)}
		{...rest}
	>
...
export const ProjectSectionText = ({
	...
	<p
		className={cn("mb-4 text-base text-muted-foreground md:text-lg", className)}
		{...rest}
	>
		{children}
	</p>
