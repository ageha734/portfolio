...
const PostH1 = ({ children, id, className, ...rest }: PostHeadingProps) => (
	<h1
		className={cn(
			"group relative mt-8 mb-4 font-extrabold font-serif text-2xl",
			className,
		)}
		id={id}
		{...rest}
...
const PostH2 = ({ children, id, className, ...rest }: PostHeadingProps) => (
	<h2
		className={cn(
			"group relative mt-8 mb-4 font-extrabold font-serif text-xl",
			className,
		)}
		id={id}
		{...rest}
...
const PostH3 = ({ children, id, className, ...rest }: PostHeadingProps) => (
	<h3
		className={cn(
			"group relative mt-8 mb-4 font-extrabold font-serif text-lg",
			className,
		)}
		id={id}
		{...rest}
...
const PostH4 = ({ children, id, className, ...rest }: PostHeadingProps) => (
	<h4
		className={cn(
			"group relative mt-8 mb-4 font-extrabold font-serif text-base",
			className,
		)}
		id={id}
		{...rest}
...
const PostLink = (props: ComponentProps<"a">) => (
	<RouterLink to={props.href || ""} {...props} />
);

const PostUl = (props: ComponentProps<"ul">) => {
	return (
		<ul className={cn("mb-4 list-disc pl-8", props.className)} {...props} />
	);
};

const PostOl = (props: ComponentProps<"ol">) => {
	return (
		<ol className={cn("mb-4 list-decimal pl-8", props.className)} {...props} />
	);
};
...
const PostBlockquote = (props: ComponentProps<"blockquote">) => {
	return (
		<blockquote
			className="my-8 border-primary border-l-4 pl-4 text-muted-foreground italic"
			{...props}
		/>
	);
};
...
const PostImage = ({
	...
	height,
	...rest
}: PostImageProps & ComponentProps<"img">) => {
	return (
		<img
