...
test.describe("ShareButton Interactions", () => {
	test("should trigger alert when clicked", async ({ page }) => {
		await page.goto(
			"/iframe.html?id=features-share-button-sharebutton--default",
		);
	...
	test("should display label when WithLabel variant is used", async ({
		page,
	}) => {
		await page.goto(
			"/iframe.html?id=features-share-button-sharebutton--with-label",
		);
	...
	test("should be disabled when Disabled variant is used", async ({ page }) => {
		await page.goto(
			"/iframe.html?id=features-share-button-sharebutton--disabled",
		);
