...
async function recoverToHomePage(
	page: Page,
	failedRoute: string,
): Promise<boolean> {
	try {
		...
		console.warn(
			`Failed to recover to home page after error on ${failedRoute}: ${recoveryErrorMessage}`,
		);
...
test.describe("Rapid Navigation", () => {
	test("should handle rapid navigation without errors", async ({ page }) => {
		...
		console.log(
			`Navigation summary: ${successCount} successful, ${errorCount} errors out of 10 attempts`,
		);
		...
		if (navigationErrors.length > 0) {
			console.log(
				"Failed routes:",
				navigationErrors.map((e) => `${e.route}: ${e.error}`).join(", "),
			);
		}

		if (errorCount / 10 > 0.5) {
			console.warn(
				"High navigation error rate detected. Some routes may be unstable.",
			);
		}
		if (recoveryFailureCount > 0 && recoveryFailureCount / errorCount > 0.5) {
			console.warn(
				"High recovery failure rate detected. Application may be in an unstable state.",
			);
		}
