...
import { createPortfolioTiDBConfig, TIDB_ALLOWED_REGIONS, TIDB_SERVERLESS_RECOMMENDATIONS, } from "./resources/databases.js";
...
import { createDopplerProject, createDopplerSecretsOnly, getCloudflareEnvVars, getSecretsFromCreatedResources, } from "./resources/secrets.js";
...
let dopplerProjectResources;
let dopplerSecretsResources;
...
export const dopplerEnvironmentIds = dopplerProjectResources
    ? pulumi.output(Object.fromEntries(Object.entries(dopplerProjectResources.environments).map(([key, env]) => [key, env.id])))
    : undefined;
let dopplerSecrets;
if (shouldCreateDopplerProject && dopplerProjectResources) {
    ...
    dopplerSecrets = {
        ...
        VITE_SENTRY_REPLAY_ON_ERROR_SAMPLE_RATE: createdSecrets.VITE_SENTRY_REPLAY_ON_ERROR_SAMPLE_RATE ?? pulumi.output(""),
    ...
    const pulumiConfigForEnv = new pulumi.Config();
    const configName = pulumiConfigForEnv.require("dopplerConfig");
    const createdSecrets = getSecretsFromCreatedResources({ secrets: dopplerSecretsResources }, configName);
    dopplerSecrets = {
        DATABASE_URL: createdSecrets.DATABASE_URL ?? pulumi.output(""),
        REDIS_URL: createdSecrets.REDIS_URL ?? pulumi.output(""),
        CLOUDFLARE_API_TOKEN: createdSecrets.CLOUDFLARE_API_TOKEN ?? pulumi.output(""),
...
new doppler.Secret("doppler-secret-auto-database-url", {
    ...
    value: pulumi
        ...
        pulumi.log.info(`[DEBUG_TRACE] >>> STATE: DATABASE_URL selection - clusterConnectionString: ${clusterConnectionString ? "exists" : "none"}, generatedUrl: ${generatedUrl ? "exists" : "none"}, existingUrl: ${existingUrl ? "exists" : "none"}`);
...
const pagesProjects = createPortfolioPagesProjects(config, {
    databaseUrl: tidb.connectionString,
    redisUrl: redis.connectionString,
}, cloudflareProvider, apiWorkerScriptName);
...
export const accessApplicationIds = previewAccess
    ? pulumi
        ...
        .apply((apps) => Object.fromEntries(Object.entries(apps).map(([key, app]) => [
        key,
        app.id,
    ])))
    : undefined;
...
export const grafanaDashboardUids = pulumi
    ...
    .apply((dashboards) => Object.fromEntries(Object.entries(dashboards).map(([key, dashboard]) => [key, dashboard.uid])));
export const sentryDsn = observability.sentry.dsn;
export const sentryTeamSlug = observability.sentry.team.slug;
export const sentryProjectSlugs = Object.fromEntries(Object.entries(observability.sentry.projects).map(([key, project]) => [key, project.slug]));
const projectName = getProjectName();
...
export const summary = {
    ...
    databases: {
        ...
        redis: redis.database && redis.subscription
            ? {
