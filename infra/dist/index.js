...
import { TIDB_ALLOWED_REGIONS, TIDB_SERVERLESS_RECOMMENDATIONS, createPortfolioTiDBConfig, } from "./resources/databases.js";
...
import { getCloudflareEnvVars, createDopplerProject, } from "./resources/secrets.js";
...
export const dopplerEnvironmentIds = dopplerProjectResources
    ? pulumi.output(Object.fromEntries(Object.entries(dopplerProjectResources.environments).map(([key, env]) => [
        key,
        env.id,
    ])))
    : undefined;
...
export const dnsRecordIds = pulumi.output(dnsRecords.records).apply((records) => Object.fromEntries(Object.entries(records).map(([key, record]) => [key, record.id])));
...
export const pagesDomainNames = pulumi
    ...
    .apply((domains) => Object.fromEntries(Object.entries(domains).map(([key, domain]) => [key, domain.domain])));
...
export const workerScriptNames = pulumi
    ...
    .apply((scripts) => Object.fromEntries(Object.entries(scripts).map(([key, script]) => [key, script.name])));
...
export const grafanaDashboardUids = pulumi
    ...
    .apply((dashboards) => Object.fromEntries(Object.entries(dashboards).map(([key, dashboard]) => [
    key,
    dashboard.uid,
])));
