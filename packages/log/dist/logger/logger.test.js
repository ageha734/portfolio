...
describe("Logger", () => {
    ...
    let consoleWarnSpy;
    let consoleErrorSpy;
    let sentryCaptureErrorSpy;
    let sentryCaptureMessageSpy;
    beforeEach(() => {
        consoleDebugSpy = vi.spyOn(console, "debug").mockImplementation(() => undefined);
        consoleInfoSpy = vi.spyOn(console, "info").mockImplementation(() => undefined);
        consoleWarnSpy = vi.spyOn(console, "warn").mockImplementation(() => undefined);
        consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => undefined);
        sentryCaptureErrorSpy = vi.spyOn(sentryClient, "captureError").mockImplementation(() => undefined);
        sentryCaptureMessageSpy = vi.spyOn(sentryClient, "captureMessage").mockImplementation(() => undefined);
    });
    ...
    describe("Sentry統合", () => {
        ...
        it("メタデータ付きログがSentryに送信される", () => {
            ...
            expect(sentryCaptureMessageSpy).toHaveBeenCalledWith("info message", "info", expect.objectContaining({
                level: LogLevel.INFO,
                userId: "123",
            }));
        });
