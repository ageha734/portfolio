...
describe("ShareButton Component", () => {
    ...
    test("should not render when isAvailable is false", () => {
        const { container } = render(_jsx(ShareButton, { onShare: mockOnShare, isAvailable: false }));
        expect(container.firstChild).toBeNull();
    });
    test("should render share icon", () => {
        render(_jsx(ShareButton, { onShare: mockOnShare }));
    ...
    test("should call onShare with options when button is clicked", () => {
        ...
        render(_jsx(ShareButton, { onShare: mockOnShare, url: url, title: title, text: text }));
        const button = screen.getByRole("button");
    ...
    test("should apply custom className", () => {
        render(_jsx(ShareButton, { onShare: mockOnShare, className: "custom-class" }));
        const button = screen.getByRole("button");
        expect(button).toHaveClass("custom-class");
    });
    test("should use custom icon src and alt", () => {
        render(_jsx(ShareButton, { onShare: mockOnShare, iconSrc: "/custom-icon.svg", iconAlt: "Custom Share" }));
