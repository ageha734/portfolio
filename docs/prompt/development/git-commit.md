...
このプロジェクトでは[Conventional Commits](https://www.conventionalcommits.org/)形式に基づいたコミットメッセージを使用します。`.commitlintrc.json`の設定に従い、一貫性のあるコミット履歴を維持します。

## コミットメッセージの形式

`.commitlintrc.json`の設定では、**コミットタイプ（type）のみが必須**です。
...
`.commitlintrc.json`で許可されている次の13種類のタイプを使用します。
