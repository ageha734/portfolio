import"./main.preset-8j9zm90b.js";

// src/main.preset.ts
import { fileURLToPath } from "node:url";
import { mergeConfig } from "vite";
function getPluginName(plugin) {
  if (typeof plugin === "object" && plugin !== null && "name" in plugin) {
    const nameValue = plugin.name;
    return typeof nameValue === "string" ? nameValue : "";
  }
  if (typeof plugin === "function") {
    const fnNameValue = plugin.name;
    return typeof fnNameValue === "string" ? fnNameValue : "";
  }
  return "";
}
function shouldExcludeRemixPlugin(plugin) {
  if (!plugin)
    return false;
  if (Array.isArray(plugin)) {
    const [pluginFn] = plugin;
    if (pluginFn) {
      const name2 = getPluginName(pluginFn);
      if (name2.toLowerCase().includes("remix")) {
        return true;
      }
    }
    return false;
  }
  const name = getPluginName(plugin);
  return name.toLowerCase().includes("remix");
}
function filterRemixPlugins(plugins) {
  return plugins.filter((plugin) => !shouldExcludeRemixPlugin(plugin));
}
function createStorybookMainPreset(options) {
  const { stories, additionalAliases = {}, additionalAddons = [] } = options;
  const sentryNodeStubPath = fileURLToPath(new URL("./helper/sentry-stub.js", import.meta.url));
  const config = {
    stories,
    addons: [
      "@storybook/addon-essentials",
      "@storybook/addon-interactions",
      "@storybook/addon-links",
      "@storybook/addon-a11y",
      ...additionalAddons
    ],
    framework: {
      name: "@storybook/react-vite",
      options: {}
    },
    core: {
      disableTelemetry: true
    },
    viteFinal(config2) {
      const filteredPlugins = filterRemixPlugins(config2.plugins || []);
      return mergeConfig({
        ...config2,
        plugins: filteredPlugins
      }, {
        resolve: {
          alias: {
            ...additionalAliases,
            "@sentry/node": sentryNodeStubPath
          }
        },
        optimizeDeps: {
          exclude: ["@sentry/node"]
        },
        server: {
          watch: {
            ignored: ["**/.cache/**", "**/docs/**"]
          }
        }
      });
    }
  };
  return config;
}
export {
  createStorybookMainPreset
};
