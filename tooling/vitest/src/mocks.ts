...
try {
	...
	if (
		"clipboard" in navigatorWithClipboard &&
		navigatorWithClipboard.clipboard
	) {
		vi.spyOn(navigatorWithClipboard.clipboard, "writeText").mockImplementation(
			async (text: string) => {
				clipboardStore = text;
			},
		);
		vi.spyOn(navigatorWithClipboard.clipboard, "readText").mockImplementation(
			async () => {
				return clipboardStore;
			},
		);
